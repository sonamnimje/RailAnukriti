				...
				{exampleRecs.map((ex, idx) => (
					<li key={`ex-${idx}`} className="border rounded-xl p-3">
						...
						<div className="flex items-center gap-2 shrink-0">
							...
							{onAccept && <button className="text-xs px-3 py-1 rounded-lg bg-green-600 text-white hover:bg-green-700" onClick={() => onAccept({ train_id: ex.train_id, action: ex.action, reason: ex.reason })}>Accept</button>}
							{onOverride && <button className="text-xs px-3 py-1 rounded-lg bg-red-600 text-white hover:bg-red-700" onClick={() => onOverride({ train_id: ex.train_id, action: ex.action, reason: ex.reason })}>Override</button>}
						</div>
					</div>
					{openExampleIdx === idx && (
						<div className="mt-2 text-xs text-gray-700 bg-gray-50 rounded-lg p-2">
